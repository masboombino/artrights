<x-allthepages-layout pageTitle="PV #{{ $pv->id }}">
    <div class="space-y-6">
        <div class="rounded-lg shadow p-6" style="background-color: #F3EBDD; border: 2px solid #193948;">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="p-4 rounded" style="background-color: #ffffff; border: 2px solid #193948; border-radius: 8px;">
                    ...
                    <p class="mb-1" style="color: #36454f;">Agent: {{ $pv->agent->user->name ?? 'N/A' }}</p>
                    @if($pv->mission)
                        <p style="color: #36454f;">Mission: {{ $pv->mission->title }}</p>
                    @endif
                </div>
        ...
        <a href="{{ route('admin.manage-pvs') }}" class="inline-block rounded px-4 py-2 font-semibold" style="background-color: #D6BFBF; color: #193948;">
            Back to list
        </a>
    </div>

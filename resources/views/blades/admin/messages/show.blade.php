<x-allthepages-layout pageTitle="Message Details">
    <div style="padding: 5px; margin: 5px;">
        <div style="margin-bottom: 1.5rem;">
            <a href="{{ route('admin.messages.index') }}" style="padding: 10px 20px; background-color: #193948; color: #4FADC0; border-radius: 0.5rem; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 8px;">
                ← Back to Messages
            </a>
        </div>
        ...
        @php
            ...
            if (!$message) {
                $message = \App\Models\Complain::with(['sender', 'targetUser', 'admin', 'gestionnaire', 'artist.user', 'agentProfile.user'])->findOrFail($id ?? request()->route('id'));
            }
        @endphp
        ...
        <div class="page-container" style="margin-bottom: 1.5rem;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: start; flex-wrap: wrap; gap: 15px; padding-bottom: 1.5rem; margin-bottom: 1.5rem; border-bottom: 3px solid #D6BFBF;">
                <div style="flex: 1;">
                    ...
                    <h1 style="color: #D6BFBF; font-size: 1.75rem; font-weight: 700; margin-bottom: 0.5rem;">{{ $message->subject }}</h1>
            ...
            @else
                ...
                <div style="margin-top: 2rem; padding-top: 2rem; border-top: 3px solid #D6BFBF;">
                    ...
                    <form action="{{ route('admin.messages.respond', $message->id) }}" method="POST" enctype="multipart/form-data" style="background-color: #F3EBDD; padding: 1.5rem; border-radius: 0.5rem; border: 2px solid #193948;">
                        @csrf
                        <div style="margin-bottom: 1rem;">
                            <label style="display: block; color: #193948; font-weight: 700; margin-bottom: 0.5rem;">Your Response *</label>
                            <textarea name="admin_response" rows="6" required style="width: 100%; padding: 0.75rem; border: 2px solid #193948; border-radius: 0.5rem; color: #193948; font-size: 1rem; background-color: white; resize: vertical;"></textarea>
                        </div>
                        <div style="margin-bottom: 1rem;">
                            <label style="display: block; color: #193948; font-weight: 700; margin-bottom: 0.5rem;">Response Images (Optional, Max 5)</label>
                            <input type="file" name="admin_response_images[]" multiple accept="image/*" style="width: 100%; padding: 0.75rem; border: 2px solid #193948; border-radius: 0.5rem; background-color: white;">
                        </div>
                        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                            ...
                            <form action="{{ route('admin.messages.resolve', $message->id) }}" method="POST" style="display: inline;">
                                @csrf

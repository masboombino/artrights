<x-allthepages-layout pageTitle="Complaints Sent">
    <div style="padding:1rem;">
        <div class="stat-card" style="padding:1rem; margin-bottom:1rem;">
            ...
            <p style="margin:0.4rem 0 0; color:#36454f;">Complaints sent by gestionnaire with reply state.</p>
        </div>
                ...
                <div style="overflow-x:auto;">
                    <table style="width:100%; min-width:900px; border-collapse:collapse;">
                        ...
                        @foreach($items as $item)
                            @php
                                $replyField = $item->target_role === 'admin' ? 'admin_response' : 'super_admin_response';
                            @endphp
                            <tr style="background:{{ $loop->odd ? '#fff' : '#f8f8f8' }};">
                                ...
                                <td style="padding:0.8rem;"><a href="{{ route('gestionnaire.complaints.show', $item->id) }}" style="padding:0.35rem 0.7rem; border-radius:10px; text-decoration:none; background:#193948; color:#4FADC0; font-weight:700;">Open</a></td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>

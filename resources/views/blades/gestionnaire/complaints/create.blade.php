<x-allthepages-layout pageTitle="Submit Complaint" :disableZoom="true">
    <div class="space-y-6">
        <div class="rounded-lg shadow p-6" style="background-color: #F3EBDD;">
            ...
            <form action="{{ route('gestionnaire.complaints.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                    ...
                    <div>
                        ...
                        <select name="target_role" id="target_role" required
                            ...
                            @foreach($targets as $target)
                                <option value="{{ $target }}" @selected(old('target_role', 'admin') === $target)>
                                    {{ ucfirst(str_replace('_', ' ', $target)) }}
                                </option>
                            @endforeach
                        </select>
                        <p class="mt-1 text-xs" style="color: #36454f;">You can reach admins or agents.</p>
                        @error('target_role')
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ route('gestionnaire.complaints.index') }}" class="rounded transition hover:opacity-90" style="background-color: #36454f; color: #4FADC0; padding: 1rem 2rem;">
                        Cancel
                    </a>
                </div>
            </form>

    ...
    <div style="padding: 5px; margin: 5px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; margin-bottom: 2rem; padding: 1rem; background: linear-gradient(135deg, #F3EBDD 0%, #D6BFBF 100%); border-radius: 1rem; border: 2px solid #193948;">
            <div>
                ...
                <p style="color: #193948; font-size: 0.9rem; margin-top: 0.5rem; opacity: 0.8;">Unified system for Complaints & Reports - System Wide</p>
            </div>
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                <a href="{{ route('superadmin.messages.create', ['type' => 'complaint']) }}" style="padding: 12px 24px; background-color: #E76268; color: white; border-radius: 0.5rem; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 8px;">
                    ⚠️ New Complaint
                </a>
                <a href="{{ route('superadmin.messages.create', ['type' => 'report']) }}" style="padding: 12px 24px; background-color: #10b981; color: white; border-radius: 0.5rem; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 8px;">
                    📊 New Report
                </a>
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 15px; margin-bottom: 2rem;">
            ...
            <div class="page-container" style="text-align: center; padding: 1.5rem; background: linear-gradient(135deg, #E0E7FF 0%, #C7D2FE 100%); border: 2px solid #6366f1;">
                <div style="font-size: 2.5rem; font-weight: 700; color: #6366f1;">{{ $stats['admin_complaints'] ?? 0 }}</div>
                <div style="color: #193948; font-size: 0.9rem; margin-top: 0.5rem;">Admin Complaints</div>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 5px; border-bottom: 3px solid #D6BFBF; margin-bottom: 1.5rem; flex-wrap: wrap; background-color: #F3EBDD; padding: 0.5rem; border-radius: 0.5rem 0.5rem 0 0;">
            ...
            <a href="{{ route('superadmin.messages.index', ['type' => 'complaint']) }}" 
            ...
            <a href="{{ route('superadmin.messages.index', ['type' => 'report']) }}" 
        ...
        <div style="display: flex; gap: 10px; margin-bottom: 1.5rem; flex-wrap: wrap; align-items: center; padding: 1rem; background-color: #F3EBDD; border-radius: 0.5rem; border: 2px solid #D6BFBF;">
            <form method="GET" action="{{ route('superadmin.messages.index') }}" style="display: flex; gap: 10px; flex-wrap: wrap; align-items: center; flex: 1;">
                @if(request('type'))
                ...
                @if(request('status') || request('type'))
                    <a href="{{ route('superadmin.messages.index') }}" style="padding: 8px 20px; background-color: #D6BFBF; color: #193948; border-radius: 0.5rem; text-decoration: none; font-weight: 600; font-size: 0.9rem;">
                        🗑️ Clear
                    </a>
                @endif
            </form>
        ...
        <div class="rounded-lg shadow overflow-hidden" style="background-color: #F3EBDD; border: 2px solid #193948;">
                        ...
                        <tbody>
                            @foreach($items as $item)
                                <tr style="border-top: 1px solid rgba(25, 57, 72, 0.1); background-color: {{ $loop->even ? '#F9F9F9' : 'white' }};">
                                    ...
                                    <td style="padding: 1rem; text-align: center;">
                                        <a href="{{ route('superadmin.messages.show', $item->id) }}" style="padding: 6px 14px; background-color: #193948; color: #4FADC0; border-radius: 0.5rem; text-decoration: none; font-size: 0.85rem; font-weight: 600;">
                                            👁️ View
                                        </a>
                                    </td>
                                </tr>
            ...
            @else
                <div style="padding: 4rem; text-align: center;">
                    ...
                    <a href="{{ route('superadmin.messages.index') }}" style="padding: 12px 24px; background-color: #193948; color: #4FADC0; border-radius: 0.5rem; text-decoration: none; font-weight: 600;">
                        View All Messages
                    </a>
                </div>
            @endif

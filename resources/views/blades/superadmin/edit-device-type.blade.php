<x-allthepages-layout pageTitle="Edit Device Type">
    <div class="max-w-2xl mx-auto">
        <div class="rounded-lg shadow p-6" style="background-color: #F3EBDD;">
            <form action="{{ route('superadmin.update-device-type', $deviceType->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select name="type" id="type"
                        ...
                        <option value="Public" @selected(old('type', $deviceType->type) === 'Public')>Public</option>
                        <option value="Commercial" @selected(old('type', $deviceType->type) === 'Commercial')>Commercial</option>
                        <option value="Personal" @selected(old('type', $deviceType->type) === 'Personal')>Personal</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm" style="color: #E76268;">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="coefficient" id="coefficient" value="{{ old('coefficient', $deviceType->coefficient) }}" step="0.1" min="0.1" required
                        class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        style="background-color: white; color: #193948;">
                    @error('coefficient')
                        <p class="mt-1 text-sm" style="color: #E76268;">{{ $message }}</p>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        style="background-color: white; color: #193948;">{{ old('description', $deviceType->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm" style="color: #E76268;">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex gap-4">
                    <button type="submit" class="rounded transition hover:opacity-90" style="background-color: #193948; color: #4FADC0; padding: 1rem 2rem; border: none; cursor: pointer; font-weight: 600;">
                        <span style="padding: 0 0.5rem;">Update Device Type</span>
                    </button>

<x-allthepages-layout pageTitle="Create New Agency">
    <div class="max-w-4xl mx-auto">
        <div class="rounded-lg shadow p-6" style="background-color: #F3EBDD;">
            <h2 class="text-2xl font-bold mb-6" style="color: #193948;">Create New Agency</h2>
            
            <form action="{{ route('superadmin.store-agency') }}" method="POST">
                @csrf
                ...
                <div class="mb-8 p-4 rounded-lg" style="background-color: rgba(255,255,255,0.5);">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            <label for="wilaya_code" class="block text-sm font-medium mb-2" style="color: #193948;">Wilaya *</label>
                            <select name="wilaya_code" id="wilaya_code" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                style="background-color: white; color: #193948;">
                                <option value="">-- Select Wilaya --</option>
                                @foreach($wilayas as $code => $name)
                                    <option value="{{ $code }}" {{ old('wilaya_code') == $code ? 'selected' : '' }}>{{ $code }} - {{ $name }}</option>
                                @endforeach
                            </select>
                            @error('wilaya_code')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="agency_name" id="agency_name" value="{{ old('agency_name') }}" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                style="background-color: white; color: #193948;"
                                placeholder="e.g., Alger Office">
                            @error('agency_name')
                                <p class="mt-1 text-sm" style="color: #E76268;">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ route('superadmin.manage-agencies') }}" class="rounded transition hover:opacity-90" style="background-color: #6c757d; color: white; padding: 1rem 2rem;">
    ...
    <script>
        // Auto-fill agency name based on wilaya selection
        document.getElementById('wilaya_code').addEventListener('change', function() {
            const selectedOption = this.options[this.selectedIndex];
            const wilayaName = selectedOption.text.split(' - ')[1];
            if (wilayaName) {
                document.getElementById('agency_name').value = wilayaName + ' Office';
            }
        });
    </script>
